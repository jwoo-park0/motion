...
from utils.query_util import obtain_query, querycache_unet_attention, queryinject_unet_attention, update_unet_query
...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        self.ddim_timesteps = make_ddim_timesteps(ddim_discr_method=ddim_discretize, num_ddim_timesteps=ddim_num_steps,
                                                num_ddpm_timesteps=self.ddpm_num_timesteps, guidance_step=self.model.config.guidance_step,
    ...
    def ddim_sampling(self, cond, shape,
        ...
        query_store_kwargs={'t_range': [0, self.ddim_timesteps//10], 'strength_start': 0.9, 'strength_end': 0.81836735}
        ...
        for i, step in enumerate(iterator):
            ...
            if step >= query_store_kwargs.t_range[0] and step <= query_store_kwargs.t_range[1]:
                self.model.diffusion_model = querycache_unet_attention(self.model.diffusion_model,*query_store_kwargs)
                obtain_query(noise_step=step,generator=kwargs['generator'])
                self.model.diffusion_model = update_unet_query(self.model.diffusion_model, step)
                self.model.diffusion_model = queryinject_unet_attention(self.model.diffusion_model)
    ...
    def guidance_p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      uc_type=None, conditional_guidance_scale_temporal=None, **kwargs):
        b, *_, device = *x.shape, x.device
        
        control_latents = x.clone().detach()
        control_latents.requires_grad = True
        
        # control_latents = self.scheduler.scale_model_input(control_latents, t) 
        # noisy_latents = self.scheduler.scale_model_input(noisy_latents, t)
        noisy_latents = x
        if x.dim() == 5:
            is_video = True
        else:
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if isinstance(c, torch.Tensor):
                e_t = self.model.apply_model(control_latents, t, c, **kwargs)
                with torch.no_grad():
                    e_t_uncond = self.model.apply_model(noisy_latents, t, unconditional_conditioning, **kwargs)
            ...
            temp_attn_prob_control = self.model.get_temp_attn_prob()
            
            loss_motion = self.model.config.motion_scale * self.compute_temp_loss(temp_attn_prob_control,)
            
            step_index = kwargs.get('i')
            if step_index < self.model.config.warm_up_steps:
                scale = (step_index+1)/self.model.config.warm_up_steps
                loss_motion = scale*loss_motion
        
            if step_index > self.model.config.guidance_steps-self.model.config.cool_up_steps:
                scale = (self.model.config.guidance_steps-step_index)/self.model.config.cool_up_steps
                loss_motion = scale*loss_motion
            
            gradient = torch.autograd.grad(loss_motion, control_latents, allow_unused=True)[0]
        ...
        guidance_scale = self.model.config.guidance_scale 
        ...
        if is_video:
            ...
            size = (b, 1, 1, 1)
        a_t = torch.full(size, alphas[index], device=device) #alpha_prod_t 
        a_prev = torch.full(size, alphas_prev[index], device=device) #alpha_prod_t_prev
        sigma_t = torch.full(size, sigmas[index], device=device)
        sqrt_one_minus_at = torch.full(size, sqrt_one_minus_alphas[index],device=device) #sqrt(beta_prod_t)
        ...
        if gradient is not None and guidance_scale > 0.0:
            assert e_t.shape == gradient.shape
            e_t = e_t - guidance_scale * (1 - a_t) ** (0.5) * gradient.detach()
