...
from utils.freq_util import apply_high_pass, apply_low_pass
...
class CrossAttention(nn.Module):
    ...
    def get_attention_scores(
        ...
        """ high frequency 값만 가져오는게 아니라 어떤 작용을 해줘야하는데 (ex. high freq enhancement / switch) \\
            근데 그런 작용 없이 그저 high freq만 가져온 버전 """
        attention_high_freq = apply_high_pass(attention_scores)
        # attention_low_freq = apply_low_pass(attention_scores)
        # attention_enhanced  = 5*attention_high_freq + 0.5*attention_low_freq

        attention_probs = attention_high_freq.softmax(dim=-1)
        # attention_probs_enhanced = attention_enhanced.softmax(dim=-1)

        del attention_scores
        del attention_high_freq

        attention_probs = attention_probs.to(dtype)
        # attention_probs_enhanced = attention_probs_enhanced.to(dtype)

        return attention_probs # attention_probs_enhanced # 
