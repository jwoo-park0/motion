...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, timesteps=None):
        ...
        ######record######
        if self.processor is not None and self.processor.mode == 'cache':
            self.processor.record_qkv(self, x, q, k, v, mask, timesteps.item())
        ##################
        
        ######inject######
        if torch.equal(context, x) :
            if self.querystore is not None and self.querystore.mode == 'cache':
                self.querystore.cache_query(q.detach().cpu(), timesteps.item())
                # torch.save(q.detach().cpu(), f'/nfs/home/wldn1677/aaai2026/VideoCrafter/query_feature/{self.__class__.__name__}_query_{timesteps.item()}.pt')
                
            if self.querystore is not None and self.querystore.mode == 'inject':
                q = self.querystore.inject_query(q, timesteps.item())
        ##################
        ...
        ######record######
        if self.processor is not None and self.processor.mode == 'inject':
            sim = self.processor.inject_attn(sim,timesteps.item())
        ##################
    ...
    def efficient_forward(self, x, context=None, mask=None, timesteps=None):
        q = self.to_q(x)
        context = default(context, x)
        ...
        if torch.equal(context, x) :
            if self.querystore is not None and self.querystore.mode == 'cache':
                self.querystore.cache_query(q.detach().cpu(), timesteps.item())
                # torch.save(q.detach().cpu(), f'/nfs/home/wldn1677/aaai2026/VideoCrafter/query_feature/{self.__class__.__name__}_query_{timesteps.item()}')
            if self.querystore is not None and self.querystore.mode == 'inject':
                q = self.querystore.inject_query(q, timesteps.item())
        ##################
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, timesteps=None):
        ...
        input_tuple = (x, timesteps)      ## should not be (x), otherwise *input_tuple will decouple x into multiple arguments
        if context is not None:
            input_tuple = (x, timesteps, context)
        if mask is not None:
            forward_mask = partial(self._forward, mask=mask, timesteps=timesteps)
            return checkpoint(forward_mask, (x,), self.parameters(), self.checkpoint)
        if context is not None and mask is not None:
            input_tuple = (x, timesteps, context, mask)
        return checkpoint(self._forward, input_tuple, self.parameters(), self.checkpoint)

    def _forward(self, x, timesteps=None, context=None, mask=None):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None, mask=mask, timesteps=timesteps) + x
        x = self.attn2(self.norm2(x), context=context, mask=mask, timesteps=timesteps) + x
...
class TemporalTransformer(nn.Module):
    ...
    def forward(self, x, context=None, timesteps=None):
        ...
        if self.only_self_att:
            ...
            for i, block in enumerate(self.transformer_blocks):
                x = block(x, mask=mask, timesteps=timesteps)
            ...
            for i, block in enumerate(self.transformer_blocks):
                ...
                for j in range(b):
                    ...
                    x[j] = block(x[j], context=context_j, timesteps=timesteps)
