...
# from lvdm.modules.attention_query import SpatialTransformer, TemporalTransformer
from lvdm.modules.attention_query import SpatialTransformer, TemporalTransformer
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, timesteps=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, batch_size, timesteps)
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb,  batch_size=None, timesteps=None):
        """
    ...
    def _forward(self, x, emb,  batch_size=None, timesteps=None):
        if self.updown:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        ...
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context, batch_size=b, timesteps=timesteps)
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context, batch_size=b, timesteps=timesteps)
        ...
        h = self.middle_block(h, emb, context=context, batch_size=b,timesteps=timesteps)
